...
# Anti-diagonal mirror FIX ekli: (x,y) -> (W-1-y, H-1-x).
# Üst-Sağ 1sn basılı: bu FIX'i toggle eder. Üst-Sol 1sn: yön sihirbazını yeniden çalıştırır.
...
def default_cfg():
    # temel eşleme; gerekirse wizard günceller
    return {
        "swap_xy": True,
        "invert_x": True,
        "invert_y": False,
        "xmin": 0, "xmax": 3840, "ymin": 0, "ymax": 3840,
        "ad_fix": True  # anti-diagonal düzeltme AÇIK
    }
...
def clamp_range(a,b):
    ...
    if hi - lo < 50:
        mid = (lo+hi)//2
        lo = max(RAW_MIN, mid-600); hi = min(RAW_MAX, mid+600)
    return lo,hi
...
def map_point(rx, ry, cfg, W, H):
    ...
    # --- anti-diagonal FIX: gözlenen hatayı düzelt ---
...
class Touch:
    ...
    # Basit 2-jest sihirbazı (sağa, aşağı). Aralık + yön çıkarır.
    ...
    def run_wizard(self, lcd, W, H):
        ...
        if not r: self._flash(lcd,W,H,"RIGHT failed",BAD); return False
        time.sleep(0.2)
        d=self._collect_swipe(lcd,W,H,"DOWN")
        if not d: self._flash(lcd,W,H,"DOWN failed",BAD); return False
        pad=80
            ...
            for ix in (False,True):
                for iy in (False,True):
                    cfg={"swap_xy":swap,"invert_x":ix,"invert_y":iy,
                         ...
                         "ad_fix": self.cfg.get("ad_fix", True)}
                    # vektörleri ekran uzayına çevir
                    sx1,sy1 = map_point(*r["start"], cfg, W, H)
                    ex1,ey1 = map_point(*r["end"],   cfg, W, H)
                    v1x,v1y = ex1-sx1, ey1-sy1
                    sx2,sy2 = map_point(*d["start"], cfg, W, H)
                    ex2,ey2 = map_point(*d["end"],   cfg, W, H)
                    v2x,v2y = ex2-sx2, ey2-sy2
                    NEG=10000
                    s=0
                    if v1x<=0: s+=NEG+abs(v1x)*10
                    if v2y<=0: s+=NEG+abs(v2y)*10
                    s+=6*abs(v1y)+6*abs(v2x)
                    s+=-2*(abs(v1x)+abs(v2y))
                    if best is None or s<best[0]: best=(s,cfg)
        self.cfg = best[1]
        CFG_PATH.write_text(json.dumps(self.cfg))
        self._flash(lcd,W,H,f"swap={self.cfg['swap_xy']} ix={self.cfg['invert_x']} iy={self.cfg['invert_y']} ad={self.cfg['ad_fix']}",GOOD,"Saved")
        return True
...
class App:
    ...
    def screen_idle(self, msg="Touch: waiting  |  TL: wizard  TR: flip"):
        ...
        d.text((8,8), msg, fill=FG); self.lcd.ShowImage(img)
            ...
            if raw:
                ...
                if x>self.W-52 and y<40:
                    if self.hold_tr is None: self.hold_tr=time.time()
                        ...
                        CFG_PATH.write_text(json.dumps(self.touch.cfg))
